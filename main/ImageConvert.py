...
def convert_color(image,color_mode):
    ...
    # Convert the average color to a grayscale value
    gray_value = sum(average_color[:3]) / 3

    # Calculate the white balance adjustment factor
    factor = 128 / gray_value

    # Apply the white balance adjustment to the image
    balanced_image = cv2.convertScaleAbs(image, alpha=factor, beta=0)'''

    if color_mode < 0 and color_mode > 5:
        image = cv2.cvtColor(image,COLOR_BayerGB2BGR)
    return image
