...
def signedurl(name, bucket='globus_screw'):
    storage_client = storage.Client.from_service_account_json(
        "/home/jupyter/globus_shivansh/suyash/faceopen-techolution-firebase-adminsdk-mq18c-0bde0112e8.json")
...
def upload_image(gcs_path, local_path, bucket='globus_screw'):
    storage_client = storage.Client.from_service_account_json(
        "/home/jupyter/globus_shivansh/suyash/storage@faceopen-techolution.iam.gserviceaccount.com.json")
    bucket = storage_client.bucket(bucket)
...
def filter_background(img_path, screw_np):
    st = time.time()
    imgx = screw_np
...
def canny_caculators_thread(req_rec, screw_np):
    image_th = screw_np
    ...
    return y_co_th, x_co_th, imgx_th, edges_th, cols_th


def canny_caculators_length(image, screw_np):
    edges = canny(cv2.cvtColor(image, cv2.COLOR_BGR2GRAY), 2, 1, 25)
    ...
    return edges, y_co, x_co, cols_bg_sep, imgx


def calculate_edges(req_rec, img_obj, target, screw_np):
    st = time.time()
    image = img_obj.copy()
    if target == "Screw":
        ...
        # targ_th = np.expand_dims(edges_th*255.0, axis=-1)
        # imgx_th = np.concatenate((targ_th, targ_th, targ_th), axis=-1)
        th_canny_thread = ThreadWithReturnValue(target=canny_caculators_thread, args=(req_rec, screw_np))
    if target == "Calibration":
        image_th = cv2.imread(req_rec.calib_path)
        image_th = cv2.cvtColor(image_th, cv2.COLOR_BGR2GRAY)
        edges_th = canny(image_th, 2, 1, 25)
        y_co_th, x_co_th = np.where(edges_th * 1.0)
        cols_th = [(y, x) for x, y in zip(x_co_th, y_co_th)]
    ...
    # length_canny = lambda image : canny(cv2.cvtColor(image, cv2.COLOR_BGR2GRAY), 2, 1, 25)
    # image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    # edges = canny(image, 2, 1, 25)
    th_canny_thread.start()
    th_canny_length = ThreadWithReturnValue(target=canny_caculators_length, args=(image, screw_np))
    th_canny_length.start()
    # t_lower = 2
    # t_upper = 150
    ...
    # print(imgx)
    if target == "Screw":
        edges, y_co, x_co, cols_bg_sep, imgx = th_canny_length.join()
        th2 = ThreadWithReturnValue(target=head_and_neck_diameter, args=(image, edges, y_co, x_co))
        th2.start()
        y_co_th, x_co_th, imgx_th, edges_th, cols_th = th_canny_thread.join()
        th1 = ThreadWithReturnValue(target=process_for_thread,
                                    args=(req_rec, edges_th, cols_th, 255 - imgx_th, cols_bg_sep, screw_np))
        th1.start()
    ...
    min_th, max_th, imgy = th1.join()
    img_head, img_neck, head_coord, neck_coord = th2.join()
    if target == "Screw":
...
def send_results_to_autoai_concat(status, results_at, model, img_obj_1, img_obj_2, label_tag, preds, orig_size, csv=' ',
    ...
    if status != 'calib':
        ...
        cv2.imwrite(f"{idx}_marked_calib.png", combined)
        img_path = f"{idx}_marked_calib.png"
    tagd = "Screw_Length"
    csv = results_at
...
def main(calib_path, screw_path):
    ...
    status = "calib"
    send_results_to_autoai_concat(status, calib_path, calib_obj, f"Calib_{datetime.datetime.now()}_Background",
                                  orig_size=str(calib_orig))
    ...
    calib_vald = calib_orig / (calib_edges[0] - calib_edges[1])
    ...
    results = {"Screw": {
        ...
        "Calib": {
            "mm/px": calib_vald,
            "Detected y co-ord": calib_edges,
            "Detected_size": screw_vald * (calib_edges[0] - calib_edges[1]),
            "Error": (calib_orig - screw_vald * (calib_edges[0] - calib_edges[1]))
        }
    }
    ...
    send_results_to_autoai_concat(screw_obj, screw_lines, f"Screw_{datetime.datetime.now()}_Edge",
                                  orig_size=str(screw_orig))
    status = "calib"
    send_results_to_autoai_concat(calib_obj, calib_lines, f"Calib_{datetime.datetime.now()}_Edge",
                                  orig_size=str(calib_orig))
    ...
    status = "calib"
    send_results_to_autoai(calib_obj, f"Calib_{datetime.datetime.now()}_Edge_Marked", str(calib_orig), str(results),
                           [[calib_edges, "Detected"]])
...
def background_process(req_rec, screw_np):
    ...
    if req_rec.pair_status:
        th1 = ThreadWithReturnValue(target=filter_background, args=(calib_path,))
        th1.start()
    th2 = ThreadWithReturnValue(target=filter_background, args=(screw_path, screw_np))
    th2.start()
    if req_rec.pair_status:
        calib_obj, calib_mask = th1.join()
    ...
    if req_rec.pair_status:
        return calib_obj, screw_obj, screw_path, calib_path
    else:
        return screw_obj, screw_path


def edge_detection(req_rec, screw_obj, screw_np, calib_obj=""):
    # status = "calib"
    # calib_edges, calib_lines = calculate_edges(calib_obj)
    # status = "screw"
    # screw_edges, screw_lines = calculate_edges(screw_obj)
    if req_rec.pair_status:
        target = "Calibration"
        th1 = ThreadWithReturnValue(target=calculate_edges, args=(req_rec, calib_obj, target,))
        th1.start()
    target = "Screw"
    th2 = ThreadWithReturnValue(target=calculate_edges, args=(req_rec, screw_obj, target, screw_np,))
    th2.start()
    if req_rec.pair_status:
        calib_edges, calib_lines = th1.join()
    ...
    if req_rec.pair_status:
        return calib_edges, screw_edges, screw_lines, calib_lines, thread_coords_screw, thread_plot
    else:
...
def measurements(req_rec, screw_edges, screw_obj, screw_lines, thread_edges, thread_plot, head_coord, neck_coord,
    ...
    print("=.=" * 20)
    print("Target Calibration Constant Thread : ", req_rec.thread_orig / abs(thread_edges[1] - thread_edges[0]))
    print("Target Calibration Constant Neck : ", req_rec.neck_orig / abs(neck_coord[1][0] - neck_coord[0][0]))
    print("Target Calibration Constant Head : ", req_rec.head_orig / abs(head_coord[1][0] - head_coord[0][0]))
    ...
    if req_rec.pair_status:
        print("Calib Orig : ", req_rec.calib_orig)
    if req_rec.pair_status:
        calib_vald = req_rec.calib_orig / (calib_edges[0] - calib_edges[1])
        results = {"Screw": {
            "mm/px": screw_vald,
            ...
            "Calib": {
                "mm/px": calib_vald,
                "Detected y co-ord": calib_edges,
                "Detected_size": round(screw_vald * (calib_edges[0] - calib_edges[1]), 4),
                "Error": (req_rec.calib_orig - screw_vald * (calib_edges[0] - calib_edges[1]))
            }
        }
        result_alt = f"""<b>Screw<b>:<br><br>
                   ...
                   <b>Calib</b>:<br><br>
                         mm/px:{calib_vald}<br>
                         Detected y co-ord:{calib_edges}<br>
                         Detected_size:{round(screw_vald * (calib_edges[0] - calib_edges[1]), 4)}<br>
                         Error:{round((req_rec.calib_orig - screw_vald * (calib_edges[0] - calib_edges[1])), 3)}<br><br>
                    """
    else:
...
def send_raw_data(req_rec, results, screw_path, calib_path, target_process, result_alt):
    ...
    calib_obj = cv2.imread(req_rec.calib_path)
    ...
    if req_rec.pair_status:
        send_results_to_autoai(model, calib_obj, f"Calib_{req_rec.trigger_time}_Raw_&_Results", str(req_rec.calib_orig),
                               str(results['Calib']['Detected_size']), target_process, str(result_alt))
...
def send_data(req_rec, results, screw_obj, result_alt, screw_lines, screw_edges, thread_edges, thread_plot, img_head,
    ...
    if req_rec.pair_status:
        send_results_to_autoai_concat("calib", str(result_alt), model, req_rec.calib_path, calib_obj,
                                      f"Calib_{req_rec.trigger_time}_Background",
                                      str(results['Calib']['Detected_size']), orig_size=str(req_rec.calib_orig))
    ...
    if req_rec.pair_status:
        send_results_to_autoai_concat("calib", str(result_alt), model, calib_obj, calib_lines,
                                      f"Calib_{req_rec.trigger_time}_Edge", str(results['Calib']['Detected_size']),
                                      orig_size=str(req_rec.calib_orig))
    ...
    send_results_to_autoai(model, screw_obj, f"Screw_{req_rec.trigger_time}_Edge_Marked", str(req_rec.screw_orig),
                           ...
                           [[screw_edges, "Detected"]])
    if req_rec.pair_status:
        send_results_to_autoai(model, calib_obj, f"Calib_{req_rec.trigger_time}_Edge_Marked", str(req_rec.calib_orig),
                               str(results['Calib']['Detected_size']), "screw_length", str(result_alt),
                               [[calib_edges, "Detected"]])
...
def fetch_thread(lc, rc, x_val, y_val):
    ...
    print("P1 : ", sel_pair)
    sel_pair_alt = (
        [sel_pair[0], list(rc)[list(rc).index(sel_pair[1]) - 1]],
        [sel_pair[0], list(rc)[list(rc).index(sel_pair[1]) - 2]])
    ...
    return sel_pair, pair_up, pair_down, sel_pair_alt, sel_pair, (
        (list(x_val)[list(y_val).index(min(y_val))], min(y_val)),
        (list(x_val)[list(y_val).index(max(y_val))], max(y_val)))


def process_for_thread(req_rec, edges, x_val, imgy, x_val_bg, screw_np):
    catl_min = {}
    catl_max = {}
    for p in x_val:
        try:
...
def main_call(req_rec, screw_np):
    sti = time.time()
    if req_rec.pair_status:
        calib_obj, screw_obj, scrw_path, calb_path = background_process(req_rec, screw_np)
        ...
        screw_obj, scrw_path = background_process(req_rec, screw_np)
        gcs_path_raw, local_path_raw = f"{datetime.datetime.now()}_Raw.png", req_rec.raw_image_path
        gcs_path_report, local_path_report = f"{datetime.datetime.now()}_Report.png", "bg_sep_screw.png"
        # upload_image(gcs_path_raw, local_path_raw)
        # upload_image(gcs_path_report, local_path_report)
    ...
    st1 = time.time()
    if req_rec.pair_status:
        calib_edges, screw_edges, screw_lines, calib_lines, thread_edges, thread_plot = edge_detection(req_rec,
                                                                                                       screw_obj,
                                                                                                       calib_obj)
    else:

        screw_edges, screw_lines, thread_edges, thread_plot, img_head, img_neck, head_coord, neck_coord = edge_detection(
            req_rec, screw_obj, screw_np)
    ...
    if req_rec.pair_status:
        ...
        th = threading.Thread(target=send_data, args=(
            req_rec, results, screw_obj, result_alt, screw_lines, screw_edges, thread_edges, thread_plot, calib_edges,
            calib_lines, calib_obj,))
        th.start()
    else:
        mst = time.time()
        results, result_alt = measurements(req_rec, screw_edges, screw_obj, screw_lines, thread_edges, thread_plot,
...
def run():
    ...
    print("Temp Time : ", time.time() - temp_st)
    ...
    if request.form['target_length'] == 'raw':
        ...
        screw = request.files['target_file']
        screw_np = np.load(screw)
        image_thread = threading.Thread(target=cv2.imwrite,args=(screw_path, screw_np))
        image_thread.start()
        print('time to read and write image',time.time()-time_to_load)
        # screw.save(screw_path)

    result, signed_url_raw, signed_url_report = main_call(req_rec, screw_np)
    # print(data)
    print("=." * 40)
    print("Total Time API Call After Main Call", time.time() - temp_st)
    print("=." * 40)
    status_c = 0
    ...
    if (result['Screw']['Thread_Diameter'] > 3.55 and result['Screw']['Thread_Diameter'] < 3.60) and (
            result['Screw']['Detected_Screw_Length'] > 17.2 and result['Screw']['Detected_Screw_Length'] < 17.5):
        status = 'Pass'
    else:
        status = 'Fail'
    ...
    'reports': [{'currentReading': '.'.join(str(result['Screw']['Detected_Screw_Length']).split('.')[:-1]+[str(result['Screw']['Detected_Screw_Length']).split('.')[-1][:3]]),
      'parameter': 'Screw Length',
      'threshold': {'min': "17.2", 'max': "17.5"}},
     {'currentReading': '.'.join(str(result['Screw']['Thread_Diameter']).split('.')[:-1]+[str(result['Screw']['Thread_Diameter']).split('.')[-1][:3]]),
      'parameter': 'Thread Diameter',
      'threshold': {'min': "3.55", 'max': "3.60"}},
     {'currentReading': "-",
      'parameter': 'Neck Diameter',
    ...
    print("=." * 40)
    print("Total Time API Call ", time.time() - temp_st)
    print("=." * 40)
    print('00000 ' * 10, time.time() - time_start)
    return jsonify({"Status": "Successful", "Data": str(result)})


if __name__ == '__main__':
    app.run(host='10.138.0.16', port=8501, threaded=True)
