...
def canny_caculators_thread(req_rec):
...
def canny_caculators_length(image):
...
def calculate_edges(req_rec, img_obj, target):
    ...
    if target == "Screw":
        ...
        # targ_th = np.expand_dims(edges_th*255.0, axis=-1)
        # imgx_th = np.concatenate((targ_th, targ_th, targ_th), axis=-1)
        th_canny_thread = ThreadWithReturnValue(target=canny_caculators_thread, args=(req_rec,))
    ...
    th_canny_length = ThreadWithReturnValue(target=canny_caculators_length, args=(image,))
    ...
    if target == "Screw":
        ...
        th1 = ThreadWithReturnValue(target=process_for_thread,
                                    args=(req_rec, edges_th, cols_th, 255 - imgx_th, cols_bg_sep,))
        th1.start()
...
def measurements(req_rec, screw_edges, screw_obj, screw_lines, thread_edges, thread_plot, head_coord, neck_coord,
    ...
    if req_rec.pair_status:
        ...
        results = {"Screw": {
            ...
            "Head_Error": req_rec.head_orig - abs(head_coord[1][0] - head_coord[0][0]) * req_rec.calib_const_neck,
        ...
        result_alt = f"""<b>Screw<b>:<br><br>
                         ...
                         Head_Error : {req_rec.head_orig - abs(head_coord[1][0] - head_coord[0][0]) * req_rec.calib_const_neck}<br>
...
def main_call(req_rec):
    sti = time.time()
    if req_rec.pair_status:
        calib_obj, screw_obj, scrw_path, calb_path = background_process(req_rec)
